...
contract TitanLegendsBattlefield is ERC721Holder, Ownable {
    using EnumerableSet for EnumerableSet.UintSet;

    IERC721A public immutable titanLegends;
    ILGNDX public immutable legendX;

    uint256 public immutable tokensPerBountyPoint = 59_644.655 ether;
    ...
    constructor(address _titanLegends, address _legendX) Ownable(msg.sender) {
        titanLegends = IERC721A(_titanLegends);
        legendX = ILGNDX(_legendX);
    }

    function dragonsAtBattle(
        address account
    ...
    function claimBounty(uint256[] calldata tokenIds) external {
        require(bountyAvailable(msg.sender), "You cannot claim bounty now");
        require(
            tokenIds.length <= maxPerBounty,
            "Max number of dragons per bounty exceeded"
        );
    ...
    function payRansom(uint256[] calldata tokenIds) external {
        uint256 multiplierPool;
        for (uint256 i; i < tokenIds.length; i++) {
            uint256 tokenId = tokenIds[i];
            require(
                _battles[msg.sender].contains(tokenId),
